 ...
 * - Signed in + Supabase configured + an ACTIVE deal → sync the deal's
 *   `deal_data` row. On first activation, single-user data is migrated into the
 *   deal (issue #70) so nothing is lost.
 * - Signed in but no active deal yet (e.g. deals still loading) → fall back to
 *   the legacy `user_data` sync, exactly as before.
